...
### Verified test counts (2026-05-23 — Fatigue Meter Phase 2 Stage 2 implementation, branch `feat/fatigue-meter-phase-2-stage-2`)
- **pytest**: 1442 passed (~3m 07s) — 1351 Stage-1 baseline + 91 new Stage-2 tests (79 unit cases in `tests/test_fatigue.py` covering per-muscle math, logged-side, period windows, SFR sentinel, Unassigned-bucket invariant; 12 integration cases in `tests/test_fatigue_routes.py` covering `GET /fatigue` empty/populated/unranked paths + the Unassigned-via-route invariant).
- **Playwright Chromium targeted (`e2e/fatigue.spec.ts`)**: 8 passed (~7.7s).
- **Full E2E Playwright (Chromium)**: 437 passed / 25 failed / 17 did-not-run on first verify-suite run. 12 of the 25 were the expected weekly-summary + session-summary visual baselines (badge gained the "View per-muscle breakdown →" link row, same pattern as PR #28 restyle) and were **re-baselined via scoped `--update-snapshots`** — re-run confirmed 12/12 green. The remaining 13 failures + 17 did-not-runs are **pre-existing on `main`** (verified by stashed-changes re-run): 2 `workout-plan.spec.ts` tests on `#muscleModeToggle` off-viewport at 1280 width (same family as the documented `nav-dropdown.spec.ts:117` known red), 10 sub-pixel visual baseline drifts on pages with zero Stage-2 surface (welcome / workout-plan / workout-log / progression / volume-splitter at desktop light/dark + 1 thumbnail), and 17 `visual-baseline-thumbnails.spec.ts` cases that need the seed-DB preflight from `e2e/scripts/prepare_visual_db.py` per `e2e/CLAUDE.md`. None of these are Stage-2 regressions.
