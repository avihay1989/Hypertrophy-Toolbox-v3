...
1. **SQL injection**: any dynamic column/table name must be validated via `validate_column_name()` / `validate_table_name()` in `routes/filters.py:142-149` before interpolation. Flag any f-string SQL with unvalidated names.
