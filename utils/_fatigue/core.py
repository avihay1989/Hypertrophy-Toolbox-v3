"""Fatigue Meter — Phase 1 core (per-set formula, session/weekly aggregation,
band classification).

Internal leaf of the ``utils.fatigue`` facade (WP2.4). Bodies moved verbatim from
the original ``utils/fatigue.py`` Phase 1 section; see that facade's module
docstring for the Stage 0 decision log (§24.B numbers, D3 raw-set convention,
lower-inclusive / upper-exclusive band threshold convention). Pure: no DB access,
no Flask imports, no ``routes`` imports.
"""
