...
1. **Only canonical `0.jpg` images are vendored.** Upstream ships two
   ...
   `1.jpg` halves the on-disk asset payload (~94 MB → ~47 MB) and avoids
   committing assets that no v1 code path reads. Alternate-angle
   `1.jpg` images can be re-imported later if a "show alternate view"
   UI affordance is ever added — see "How to refresh" below.
