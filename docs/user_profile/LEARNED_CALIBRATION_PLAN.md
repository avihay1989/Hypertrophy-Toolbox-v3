...
> **IMPLEMENTED — PR open, not yet on `main` (branch `feat/calibration-phase-2d-c-manual-nudge`).** The third 2D slice from the split (§"Phase 2D Recommended Implementation Split" item 3). Strictly the optional manual-adjustment affordance: the user must explicitly accept any change, it populates Workout Controls inputs only, and it persists nothing — the same client-side-only contract as the MVP `Apply suggestion`. No auto-apply.
