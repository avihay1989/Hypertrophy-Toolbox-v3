...
**Remaining learned-calibration work:** Phase 2D design review is complete (owner answers locked, `38d3b1c`); **Phase 2D-A shipped to `origin/main`** (PR #56, squash `39fdd17`) and **Phase 2D-B is implemented on the open PR branch** `feat/calibration-phase-2d-b-progression-context`. Still open (do not start): **2D-C** (optional manual-adjustment affordance, still no auto-apply), **2D-D** (actual suggestion modification — gated on owner approval + Stage 4 evidence). Split + decisions: [`docs/user_profile/LEARNED_CALIBRATION_PLAN.md`](user_profile/LEARNED_CALIBRATION_PLAN.md) §"Phase 2D-B Progression Fatigue Context Surface".
...
`feat/calibration-phase-2d-b-progression-context` (PR open), branched from `main` @ `acb67b0` with the Phase 2D-B working tree. `main` is **in sync with `origin/main` (0 commits ahead)**, tip `acb67b0` (docs refresh after the PR #56 Phase 2D-A squash `39fdd17`). The learned-calibration track merged on top of the prior 2026-05-29 handover-sync state: `0f8b4b7` (PR #53 Phase 2A), `bad70c6` (PR #54 Phase 2B), `d3cb404` (PR #55 Phase 2C), `eb4dbf6` (docs close-out), `39fdd17` (PR #56 Phase 2D-A). The 2D-B branch carries the 8 source/test files plus these 3 docs; `data/database.db` runtime dirt is owner-approved and **never committed** (per `CLAUDE.md` agents-must-not list — it was never staged). Feature branches `feat/calibration-phase-2d-fatigue-context` (PR #56), `feat/calibration-phase-2c-promote-profile` (PR #55), `feat/calibration-phase-2b` (PR #54), and the Phase 2A branch were merged and deleted; earlier `feat/body-composition-issue-21` (PR #31), `feat/fatigue-meter-phase-2` (PR #34), and `feat/fatigue-meter-phase-2-stage-2` (PR #35) likewise merged and deleted.
