...
**As of 2026-06-09 (latest session): active branch is `main`, in sync with `origin/main` (0 commits ahead), tip `9fb1337`.** Phase 2D-B (Progression-page advisory fatigue context) **shipped to `origin/main`** via PR #57 (squash `9fb1337`, merged 2026-06-09); feature branch `feat/calibration-phase-2d-b-progression-context` merged and deleted. Phase 2D-A shipped via PR #56 (squash `39fdd17`, 2026-06-08). The entire learned-calibration MVP + Phase 2A/2B/2C/2D-A/2D-B track is now **shipped to `origin/main`**.
