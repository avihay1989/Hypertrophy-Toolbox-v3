...
**Remaining learned-calibration work:** Phase 2D design review is complete (owner answers locked, `38d3b1c`); **Phase 2D-A shipped** (PR #56, squash `39fdd17`), **Phase 2D-B shipped** (PR #57, squash `9fb1337`), and **Phase 2D-C is implemented with a PR open** (branch `feat/calibration-phase-2d-c-manual-nudge` — see the top block). Still open (do not start): **2D-D** (actual suggestion modification — gated on owner approval + Stage 4 evidence). Split + decisions: [`docs/user_profile/LEARNED_CALIBRATION_PLAN.md`](user_profile/LEARNED_CALIBRATION_PLAN.md) §"Phase 2D-C Manual Adjustment Affordance".
...
Working on `feat/calibration-phase-2d-c-manual-nudge` (Phase 2D-C manual nudge controls — PR open, not yet merged), cut from `main` @ `f694333`. `main` is **in sync with `origin/main` (0 commits ahead)**, tip `9fb1337` (PR #57 Phase 2D-B progression fatigue context, squash merged 2026-06-09). The learned-calibration track merged on top of the prior 2026-05-29 handover-sync state: `0f8b4b7` (PR #53 Phase 2A), `bad70c6` (PR #54 Phase 2B), `d3cb404` (PR #55 Phase 2C), `eb4dbf6` (docs close-out), `39fdd17` (PR #56 Phase 2D-A), `acb67b0` (2D-A docs refresh), `9fb1337` (PR #57 Phase 2D-B). Feature branch `feat/calibration-phase-2d-b-progression-context` merged and deleted (local + remote). `data/database.db` runtime dirt is owner-approved and **never committed** (per `CLAUDE.md` agents-must-not list — it was never staged). Feature branches `feat/calibration-phase-2d-fatigue-context` (PR #56), `feat/calibration-phase-2c-promote-profile` (PR #55), `feat/calibration-phase-2b` (PR #54), and the Phase 2A branch were merged and deleted; earlier `feat/body-composition-issue-21` (PR #31), `feat/fatigue-meter-phase-2` (PR #34), and `feat/fatigue-meter-phase-2-stage-2` (PR #35) likewise merged and deleted.
