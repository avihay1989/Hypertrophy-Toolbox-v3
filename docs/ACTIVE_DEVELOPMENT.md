...
**As of 2026-06-08 (latest session): active branch is `main`, in sync with `origin/main` (0 commits ahead).** Phase 2D-A (advisory fatigue context) **shipped to `origin/main`** via PR #56 (squash `39fdd17`, merged 2026-06-08), bundling the four-commit local stack (`facd241` advisory fatigue context + `34840ee` 2D-A technical plan + `38d3b1c` 2D owner decisions + docs refresh) on top of the Phase 2C baseline `eb4dbf6`. Local `main` landed on the squash via `git reset --mixed origin/main`; the feature branch `feat/calibration-phase-2d-fatigue-context` was deleted locally **and** remotely. The entire learned-calibration MVP + Phase 2A/2B/2C/2D-A track is now **shipped to `origin/main`**.
...
**Remaining learned-calibration work:** Phase 2D design review is complete (owner answers locked, `38d3b1c`) and **Phase 2D-A shipped to `origin/main`** (PR #56, squash `39fdd17`, above). Still open (do not start): **2D-B** (broader app-wide fatigue-context surfaces), **2D-C** (optional manual-adjustment affordance, still no auto-apply), **2D-D** (actual suggestion modification — gated on owner approval + Stage 4 evidence). Split + decisions: [`docs/user_profile/LEARNED_CALIBRATION_PLAN.md`](user_profile/LEARNED_CALIBRATION_PLAN.md) §"Phase 2D-A Technical Implementation Plan".
...
`main`, **in sync with `origin/main` (0 commits ahead)**. Current tip is `39fdd17` (PR #56 Phase 2D-A advisory fatigue context, squash merged 2026-06-08). The learned-calibration track merged on top of the prior 2026-05-29 handover-sync state: `0f8b4b7` (PR #53 Phase 2A), `bad70c6` (PR #54 Phase 2B), `d3cb404` (PR #55 Phase 2C), `eb4dbf6` (docs close-out), `39fdd17` (PR #56 Phase 2D-A). Expected working-tree dirt after this docs refresh is only `data/database.db` runtime dirt (owner-approved DB dirt kept per `CLAUDE.md` agents-must-not list; **do not commit `data/database.db`** — it was never staged). Feature branches `feat/calibration-phase-2d-fatigue-context` (PR #56), `feat/calibration-phase-2c-promote-profile` (PR #55), `feat/calibration-phase-2b` (PR #54), and the Phase 2A branch were merged and deleted; earlier `feat/body-composition-issue-21` (PR #31), `feat/fatigue-meter-phase-2` (PR #34), and `feat/fatigue-meter-phase-2-stage-2` (PR #35) likewise merged and deleted.
