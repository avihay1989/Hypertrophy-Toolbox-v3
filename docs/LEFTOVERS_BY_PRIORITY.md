...
- **CI/CD Improvement Plan** — all phases shipped (PRs #40–#51). Optional fast-follows A7–A11 closed (A11 = `e2e-functional` sharding PR #71; A8 F401 flip fully closed PR #74 `18de45e`); only A12 misc remains.
...
*Last updated: 2026-06-11 (v16 — Opus closed P2 A8 fully: flipped flake8 F401 to blocking after a 56→0 unused-import cleanup + stray `replace.py` deletion, PR #74 `18de45e`; no measure-only flake8 flip candidates remain. v15 closed A11 `e2e-functional` sharding, PR #71 `17b9d7f`.) Closed sections 0/1/2/4/5/6 retained as audit trail. Live facts verified: `workout_log` = 0 rows; `main`/`origin/main` tip `18de45e` (PRs #73 docs sync + #74 F401 flip merged).*
