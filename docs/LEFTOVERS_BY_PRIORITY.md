...
| A8 | **Flip flake8 rules to blocking** | ✅ **DONE 2026-06-10.** Added `F811,E711,E712` to the blocking `--select` in `.github/workflows/ci.yml`'s "Lint with flake8" step (they sat at ~0) and aligned that step's `--exclude` with the measure-only `EXCL`. The measure-only diagnostics now track **F401 only** as the remaining flip candidate — its flip still needs a guardrailed cleanup PR (`utils/__init__.py` re-exports + `app.py` side-effects are load-bearing). Also fixed `tests/test_plan_generator.py:594` `== True` → `is True` (E712). |
...
*Last updated: 2026-06-11 (v15 — Opus closed P2 A11: sharded `e2e-functional` to n=2 via a matrix shard job + an `E2E Functional (Chromium)` fan-in gate, no Playwright-config or branch-protection change). Closed sections 0/1/2/4/5/6 retained as audit trail. Live facts verified: `workout_log` = 0 rows; `main`/`origin/main` tip before the A11 branch `cf63ef9` (PR #70 merged).*
